...
function Clinic({ back }: { back: () => void }) {
  ...
  const addAvailability=async()=>{setManagementError("");setManagementSaving(true);try{const response=await fetch("/api/clinic/management",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"availability",doctorId:Number(slotDoctorId),startsAt:slotStartsAt?new Date(slotStartsAt).toISOString():""})});const data=await response.json();if(!response.ok)throw new Error(data.error);setSlotStartsAt("");await loadManagement();}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible abrir el horario.")}finally{setManagementSaving(false)}};
