...
function Clinic({ back }: { back: () => void }) {
  ...
  const [management,setManagement]=useState<{clinic:{name:string;address:string;whatsapp:string};doctors:Array<{id:number;fullName:string;specialty:string;status:string}>;availability:Array<{id:number;doctorId:number;doctorName:string;startsAt:string;status:string}>;staff:Array<{id:number;fullName:string;email:string;role:string;status:string}>}|null>(null);
  ...
  const [rescheduleAvailabilityId,setRescheduleAvailabilityId]=useState("");
  ...
  const loadManagement=()=>fetch("/api/clinic/management").then(async response=>{const data=await response.json();if(!response.ok)throw new Error(data.error);setManagement(data);setClinicName(data.clinic.name);setClinicAddress(data.clinic.address);setClinicWhatsapp(data.clinic.whatsapp);}).catch(error=>setManagementError(error instanceof Error?error.message:"No fue posible cargar la configuración."));
  ...
  const updateAppointment=async(id:number,status:"confirmed"|"cancelled"|"reschedule_requested")=>{await fetch("/api/appointments",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,status})});loadAppointments()};
  const reassignAppointment=async()=>{setManagementError("");setManagementSaving(true);try{const response=await fetch("/api/appointments",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"reassign",id:Number(rescheduleAppointmentId),availabilityId:Number(rescheduleAvailabilityId)})});const data=await response.json();if(!response.ok)throw new Error(data.error);setRescheduleAppointmentId("");setRescheduleAvailabilityId("");await Promise.all([loadAppointments(),loadManagement()]);}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible reprogramar la cita.")}finally{setManagementSaving(false)}};
  const addDoctor=async()=>{setManagementError("");setManagementSaving(true);try{const response=await fetch("/api/clinic/management",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(editingDoctorId?{action:"update_doctor",id:editingDoctorId,fullName:doctorName,specialty:doctorSpecialty,status:"active"}:{action:"doctor",fullName:doctorName,specialty:doctorSpecialty})});const data=await response.json();if(!response.ok)throw new Error(data.error);setDoctorName("");setDoctorSpecialty("");setEditingDoctorId(null);await loadManagement();}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible guardar el médico.")}finally{setManagementSaving(false)}};
  const editDoctor=(doctor:{id:number;fullName:string;specialty:string})=>{setEditingDoctorId(doctor.id);setDoctorName(doctor.fullName);setDoctorSpecialty(doctor.specialty);setManagementError("")};
  const setDoctorStatus=async(doctor:{id:number;fullName:string;specialty:string},status:"active"|"inactive")=>{setManagementError("");setManagementSaving(true);try{const response=await fetch("/api/clinic/management",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"update_doctor",id:doctor.id,fullName:doctor.fullName,specialty:doctor.specialty,status})});const data=await response.json();if(!response.ok)throw new Error(data.error);await loadManagement();}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible actualizar el médico.")}finally{setManagementSaving(false)}};
  const setAvailabilityStatus=async(id:number,status:"available"|"blocked")=>{setManagementError("");setManagementSaving(true);try{const response=await fetch("/api/clinic/management",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"update_availability",id,status})});const data=await response.json();if(!response.ok)throw new Error(data.error);await loadManagement();}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible actualizar el horario.")}finally{setManagementSaving(false)}};
  ...
  const addRecurringAvailability=async()=>{setManagementError("");setManagementSaving(true);try{const startsAt=slotDate&&slotTime?new Date(`${slotDate}T${slotTime}:00`).toISOString():"";const response=await fetch("/api/clinic/management",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"recurring_availability",doctorId:Number(slotDoctorId),startsAt,weeks:Number(recurringWeeks)})});const data=await response.json();if(!response.ok)throw new Error(data.error);await loadManagement();}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible abrir los horarios semanales.")}finally{setManagementSaving(false)}};
  const saveClinic=async()=>{setManagementError("");setManagementSaving(true);try{const response=await fetch("/api/clinic/management",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"clinic",clinicName, address:clinicAddress, whatsapp:clinicWhatsapp})});const data=await response.json();if(!response.ok)throw new Error(data.error);await loadManagement();}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible guardar los datos de la clínica.")}finally{setManagementSaving(false)}};
  const addStaff=async()=>{setManagementError("");setManagementSaving(true);try{const response=await fetch("/api/clinic/management",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"staff",fullName:teamName,email:teamEmail,password:teamPassword})});const data=await response.json();if(!response.ok)throw new Error(data.error);setTeamName("");setTeamEmail("");setTeamPassword("");await loadManagement();}catch(error){setManagementError(error instanceof Error?error.message:"No fue posible crear el acceso.")}finally{setManagementSaving(false)}};
  const allActiveAppointments = realAppointments.filter(item => item.status !== "cancelled");
  const activeAppointments = allActiveAppointments.filter(item =>
    (!agendaDate || item.startsAt.slice(0, 10) === agendaDate) &&
    (!agendaDoctor || item.doctorName === agendaDoctor) &&
    (!agendaStatus || item.status === agendaStatus)
  );
  ...
  return <main>
    ...
    <section className="clinic">
      ...
      {tab==="Agenda"&&<div className="agenda-filters"><label>Fecha<input type="date" value={agendaDate} onChange={event=>setAgendaDate(event.target.value)} /></label><label>Médico<select value={agendaDoctor} onChange={event=>setAgendaDoctor(event.target.value)}><option value="">Todos los médicos</option>{(management?.doctors??[]).map(doctor=><option key={doctor.id} value={doctor.fullName}>{doctor.fullName}</option>)}</select></label><label>Estado<select value={agendaStatus} onChange={event=>setAgendaStatus(event.target.value)}><option value="">Todos los estados</option><option value="pending">Pendiente</option><option value="confirmed">Confirmada</option><option value="reschedule_requested">Reprogramación solicitada</option></select></label><button onClick={()=>{setAgendaDate("");setAgendaDoctor("");setAgendaStatus("")}}>Limpiar filtros</button></div>}
      {tab==="Agenda"&&<section id="reprogramar-cita" className="settings reschedule-form"><h2>Reprogramar una cita</h2><p><small>{rescheduleAppointmentId?"Elige el nuevo cupo y confirma el cambio.":"Pulsa “Reprogramar” en una cita o selecciónala aquí."}</small></p><label>Cita<select value={rescheduleAppointmentId} onChange={event=>setRescheduleAppointmentId(event.target.value)}><option value="">Selecciona una cita</option>{allActiveAppointments.map(item=><option key={item.id} value={item.id}>{item.patientName} · {item.doctorName} · {formatSlot(item.startsAt)}</option>)}</select></label><label>Nuevo horario<select value={rescheduleAvailabilityId} onChange={event=>setRescheduleAvailabilityId(event.target.value)}><option value="">Selecciona un cupo disponible</option>{(management?.availability??[]).filter(slot=>slot.status==="available").map(slot=><option key={slot.id} value={slot.id}>{slot.doctorName} · {formatSlot(slot.startsAt)}</option>)}</select></label><button className="primary" disabled={managementSaving||!rescheduleAppointmentId||!rescheduleAvailabilityId} onClick={reassignAppointment}>{managementSaving?"Guardando...":"Confirmar reprogramación"}</button></section>}
      {tab==="Agenda"&&<section className="live-agenda"><h2>Agenda real</h2>{activeAppointments.length===0?<p>No hay citas activas por ahora.</p>:activeAppointments.map(item=><article key={item.id}><em>{new Date(item.startsAt).getHours().toString().padStart(2,"0")}</em><div><small>{new Date(item.startsAt).toLocaleString("es-PA")}</small><h3>{item.patientName}</h3><p>{item.doctorName} · {item.specialty}<br/><strong>● {formatAppointmentStatus(item.status)}</strong></p></div><button onClick={()=>{setRescheduleAppointmentId(String(item.id));setRescheduleAvailabilityId("");setTimeout(()=>document.getElementById("reprogramar-cita")?.scrollIntoView({behavior:"smooth",block:"center"}),0)}}>Reprogramar</button><button onClick={()=>updateAppointment(item.id,"cancelled")}>Cancelar</button></article>)}{cancelledAppointments.length>0&&<><h2>Historial de cancelaciones</h2>{cancelledAppointments.map(item=><article key={item.id}><em>{new Date(item.startsAt).getHours().toString().padStart(2,"0")}</em><div><small>{new Date(item.startsAt).toLocaleString("es-PA")}</small><h3>{item.patientName}</h3><p>{item.doctorName} · {item.specialty}<br/><strong>● Cancelada</strong></p></div></article>)}</>}</section>}
      {content}
      {tab==="Horarios"&&<section className="settings recurring-schedule"><h2>Abrir horario recurrente</h2><p><small>Usa el médico, fecha y hora seleccionados arriba. Se abrirá el mismo día y hora cada semana.</small></p><label>Semanas<select value={recurringWeeks} onChange={event=>setRecurringWeeks(event.target.value)}>{[2,3,4,6,8,12].map(weeks=><option value={weeks} key={weeks}>{weeks} semanas</option>)}</select></label><button className="primary" disabled={managementSaving||!slotDoctorId||!slotDate||!slotTime} onClick={addRecurringAvailability}>{managementSaving?"Guardando...":"Abrir horarios recurrentes"}</button></section>}
      {tab==="Configuración"&&<section className="settings clinic-edit"><h2>Editar información pública</h2><label>Nombre de la clínica<input value={clinicName} onChange={event=>setClinicName(event.target.value)} placeholder="Ej. Clínica Familiar"/></label><label>Teléfono WhatsApp<input value={clinicWhatsapp} onChange={event=>setClinicWhatsapp(event.target.value)} placeholder="Ej. +507 6000-0000"/></label><label>Dirección<input value={clinicAddress} onChange={event=>setClinicAddress(event.target.value)} placeholder="Dirección de la clínica"/></label><p><small>Estos datos se mostrarán a los pacientes cuando busquen y reserven una cita.</small></p><button className="primary" disabled={managementSaving} onClick={saveClinic}>{managementSaving?"Guardando...":"Guardar cambios"}</button></section>}
      {tab==="Configuración"&&staff?.role==="admin"&&<section className="settings clinic-edit"><h2>Accesos del equipo</h2><p><small>Crea una cuenta de recepción independiente. Esa persona podrá iniciar sesión sin usar tu contraseña.</small></p><label>Nombre completo<input value={teamName} onChange={event=>setTeamName(event.target.value)} placeholder="Ej. María González"/></label><label>Correo<input type="email" value={teamEmail} onChange={event=>setTeamEmail(event.target.value)} placeholder="recepcion@clinica.com"/></label><label>Contraseña temporal<input type="password" value={teamPassword} onChange={event=>setTeamPassword(event.target.value)} placeholder="Mínimo 10 caracteres"/></label><button className="primary" disabled={managementSaving} onClick={addStaff}>{managementSaving?"Guardando...":"Crear acceso de recepción"}</button>{(management?.staff??[]).map(member=><p className="team-member" key={member.id}><b>{member.fullName}</b><br/><small>{member.email} · {member.role==="admin"?"Administradora":"Recepción"}</small></p>)}</section>}
      {tab==="Médicos"&&editingDoctorId&&<p className="editing-note">Estás editando un médico: modifica los campos de arriba y pulsa “Agregar médico” para guardar los cambios.</p>}
      {tab==="Médicos"&&<section className="operation-list"><h2>Administrar médicos</h2>{(management?.doctors??[]).map(doctor=><article key={doctor.id}><div><h3>{doctor.fullName}</h3><p>{doctor.specialty} · <strong>● {doctor.status==="active"?"Activo":"Inactivo"}</strong></p></div><button onClick={()=>editDoctor(doctor)}>Editar</button><button className={doctor.status==="active"?"danger":""} onClick={()=>setDoctorStatus(doctor,doctor.status==="active"?"inactive":"active")}>{doctor.status==="active"?"Desactivar":"Activar"}</button></article>)}</section>}
      {tab==="Horarios"&&<section className="operation-list"><h2>Administrar horarios</h2>{(management?.availability??[]).map(slot=><article key={slot.id}><div><h3>{slot.doctorName}</h3><p>{formatSlot(slot.startsAt)} · <strong>● {slot.status==="available"?"Disponible":slot.status==="blocked"?"Bloqueado":"Reservado"}</strong></p></div>{slot.status!=="reserved"&&<button className={slot.status==="available"?"danger":""} onClick={()=>setAvailabilityStatus(slot.id,slot.status==="available"?"blocked":"available")}>{slot.status==="available"?"Bloquear":"Reabrir"}</button>}</article>)}</section>}
      {tab==="Agenda"&&<section className="operation-list"><h2>Acciones de agenda</h2>{activeAppointments.map(item=><article key={item.id}><div><h3>{item.patientName}</h3><p>{item.doctorName} · {formatAppointmentStatus(item.status)}</p></div>{item.status==="pending"&&<button className="primary action-button" onClick={()=>updateAppointment(item.id,"confirmed")}>Confirmar cita</button>}</article>)}</section>}
    </section>
  </main>;
}
